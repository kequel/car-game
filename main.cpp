...
#define FRAME_STATE 1      // Zwyk³a aktualizacja pozycji
#define FRAME_COLLISION 2  // Powiadomienie o kolizji
	...
	while (1)
	{
		...
		if (frame.iID != my_car->iID)          // jeœli to nie mój w³asny obiekt
		{
			if (frame.type == FRAME_COLLISION && frame.iID_receiver == my_car->iID)
			{
				...
				my_car->collision_point = other_cars[frame.iID]->state.vPos;
			}
...
void InteractionInitialisation()
{
	...
	multi_reciv = new multicast_net("224.12.12.16", 10001);      // obiekt do odbioru ramek sieciowych
	multi_send = new multicast_net("224.12.12.16", 10001);       // obiekt do wysy³ania ramek
...
void VirtualWorldCycle()
{
	...
	{                              // nale¿y na nowo obliczyæ œredni czas cyklu avg_cycle_time
		...
		sprintf(text, "WZR-lab 2025/26 (lato) temat 1, wersja g (%0.0f fps  %0.2fms) ", fFps, 1000.0 / fFps);
	...
	for (map<int, MovableObject*>::iterator it = other_cars.begin(); it != other_cars.end(); ++it) {
		...
		if (dist < (my_car->bounding_radius + it->second->bounding_radius)) {
			collision_found = true;
			my_car->collision_point = it->second->state.vPos;

			Frame coll_frame;
			coll_frame.iID = my_car->iID;
			coll_frame.type = FRAME_COLLISION;
			coll_frame.iID_receiver = it->first;
			multi_send->send((char*)&coll_frame, sizeof(Frame));
			break;
		}
	}
	my_car->is_collided = collision_found;
